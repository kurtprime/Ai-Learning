...
function verifyWebhook({
  ...
  return new Webhook(process.env.WEBHOOK_SECRET_KEY!).verify(raw, headers);
}

export const clerkCreateUser = inngest.createFunction(
  { id: "clerk/create-database-user", name: "Clerk - Create Database User" },
  {
  ...
  async ({ event, step }) => {
    ...
    const userId = await step.run("create-user", async () => {
      ...
      if (!email) throw new NonRetriableError("No primary Email found");
...
export const clerkCreateOrganization = inngest.createFunction(
  {
    id: "clerk/create-database-organization",
