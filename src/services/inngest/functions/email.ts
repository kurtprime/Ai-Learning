...
export const prepareDailyUserJobListingNotifications = inngest.createFunction(
  {
    ...
    name: "Prepare Daily User Job Listing Notification",
  },
...
export const sendDailyUserJobListingEmail = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    let matchingJobListing: typeof jobListings = [];

    if (aiPrompt == null || aiPrompt.trim() === "") {
      matchingJobListing = jobListings;
    } else {
      const matchingId = await getMatchingJobListings(aiPrompt, jobListings);
      matchingJobListing = jobListings.filter((listing) =>
        matchingId.includes(listing.id)
      );
    }

    if (matchingJobListing.length === 0) return;

    await step.run("send-email", async () => {
      await resend.emails.send({
        from: "Job Board <quejadakurt@gmail.com>",
        to: user.email,
